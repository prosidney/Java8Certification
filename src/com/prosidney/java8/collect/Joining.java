...
public class Joining {
    ...
    public static void main(String[] args) {
        ...
        String collect = movies.stream().map(
                CollectWithMapping.Movie::getName)
                .collect(Collectors.joining(",","prefix","movies"));
        System.out.println(collect);


        final Map<Character, Set<String>> collect3 =
                movies.stream().collect(
                        Collectors.groupingBy(CollectWithMapping.Movie::getRating,
                                Collectors.mapping(CollectWithMapping.Movie::getName, Collectors.toSet())
                        ));
